...
import { ArrowLeft, LogIn, Mail, User as UserIcon, CheckCircle2, AlertTriangle, Loader2 } from 'lucide-react';
import { supabase } from "@/integrations/supabase/client";
import { useNavigate } from 'react-router-dom';
import { useToast } from "@/hooks/use-toast";
import { Separator } from "@/components/ui/separator";
// Removed unused Avatar imports if not needed elsewhere in the component
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        // Optional: Handle SIGNED_OUT if needed
        // if (event === "SIGNED_OUT") {
        //   // maybe clear some state or redirect to auth
        // }
      }
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Toast and navigation are handled by the onAuthStateChange listener now
        // toast({
        //   title: "Welcome back!",
        //   description: "You have successfully logged in.",
        //   duration: 3000,
        // });
        // navigate('/'); // Let the listener handle navigation
      }
    } catch (err: any) {
      ...
      <Card className="w-full max-w-md border border-festival-golden/30 dark:border-festival-golden/20 shadow-lg">
        <CardHeader className="pb-3">
          <div className="flex items-center space-x-2">
            {/* Removed back button for simplicity unless specifically needed
            <Button
              variant="ghost"
              size="icon"
              onClick={() => navigate('/')}
              className="text-festival-maroon hover:text-festival-maroon/80 dark:text-festival-golden dark:hover:text-festival-golden/80"
            >
              <ArrowLeft size={16} />
            </Button>
            */}
             ...
             <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      // Optional: Add validation pattern or length if desired
                    />
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2">
                <Label htmlFor="password" className="text-sm font-medium text-gray-700 dark:text-gray-300">Password</Label>
                 {/* Consider adding a password visibility toggle here */}
                <Input
                  id="password"
                  type="password"
                  ...
                  required
                  minLength={6} // Keep Supabase minimum password length in mind
              ...
              >
                 {googleLoading ? (
                    ...
                    <svg className="mr-2 h-4 w-4" aria-hidden="true" focusable="false" data-prefix="fab" data-icon="google" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 488 512"><path fill="currentColor" d="M488 261.8C488 403.3 381.5 512 244 512 109.8 512 0 402.2 0 261.8 0 120.3 109.8 8.8 244 8.8c77.7 0 142 31.4 190.1 78.4l-73.1 73.1c-30.8-28.6-70.1-46.3-117-46.3-89.7 0-162.4 72.7-162.4 162.4s72.7 162.4 162.4 162.4c101.5 0 138-73.9 143.4-110.3H244v-91.1h236.1c2.3 12.7 3.9 26.4 3.9 40.8z"></path></svg>
                 )}
        ...
        {!success && (
          <CardFooter className="flex justify-center pt-4">
            <Button
              ...
              className="text-sm text-festival-maroon hover:text-festival-maroon/80 dark:text-festival-golden dark:hover:text-festival-golden/80"
            >
              {isSignUp

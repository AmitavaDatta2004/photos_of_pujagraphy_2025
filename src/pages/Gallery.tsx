...
const Gallery = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 640) {
        setImagesPerPage(4) // Show fewer images on mobile
      } else {
        setImagesPerPage(8)
      }
    }
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors duration-300">
      <section className="pt-6 sm:pt-10 pb-12 sm:pb-20 bg-festival-cream/30 dark:bg-gray-800/30 min-h-screen transition-colors duration-300">
        <div className="festival-container px-4 sm:px-6">
          {/* Header with navigation and auth */}
          <GalleryHeader user={user} loading={loading} />

          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-festival-maroon dark:text-festival-golden mb-2 sm:mb-3">
            Moments of Pujagraphy
          </h1>
          <p className="text-sm sm:text-base text-center text-gray-600 dark:text-gray-300 max-w-2xl mx-auto mb-4 sm:mb-6">
            A visual celebration of Maa Durga and the culture that surrounds Her
          </p>
          ...
          {/* Pagination */}
          <GalleryPagination currentPage={currentPage} totalPages={totalPages} onPageChange={paginate} />
